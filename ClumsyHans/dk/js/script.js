...
window.addEventListener('DOMContentLoaded', function () {
  ...
  function changeFaq(id) {
    ...
    if (faqButtons[id].classList.contains('faq__plus_active')) {
      var height = parseInt(questions[id].clientHeight) + parseInt(answers[id].clientHeight) + 55;
      blocks[id].style.height = "".concat(height, "px");
    } else {
      var _height = parseInt(questions[id].clientHeight) + 30;

      blocks[id].style.height = "".concat(_height, "px");
    }
  }

  faqButtons.forEach(function (button, id) {
    button.addEventListener('click', function (e) {
      button.classList.toggle('faq__plus_active');
      changeFaq(id);
    });
  });
  document.querySelectorAll('.faq__block').forEach(function (item, id) {
    changeFaq(id);
  }); // Hamburger

  var upper = document.querySelector('.upper__wrapper'),
  ...
  var inspiration = document.getElementById('insp');
